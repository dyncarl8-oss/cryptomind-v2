...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/admin/memberships", async (req, res) => {
    try {
      ...
      // Production mode: use stored members from database
      // NOTE: We use database instead of Whop API to avoid 403 permission errors
      // Members are already captured during payment processing via /api/credits/process-payment
      if (!isWhopEnabled) {
      ...
      console.log(`[Memberships] Fetching stored members for admin ${user.userId}`);

      const limit = parseInt(req.query.limit as string) || 50;

      // Get stored members for THIS admin from database (multi-tenant)
      const storedMembers = await storage.getStoredMembersByAdmin(user.userId, ["active", "trialing", "completed"]);

      console.log(`[Memberships] Admin ${user.userId} has ${storedMembers.length} stored members`);

      // Transform stored members to match the expected membership response format
      const memberships = storedMembers.slice(0, limit).map(member => ({
      ...
      console.log(`[Memberships] Returning ${memberships.length} memberships for admin ${user.userId}`);

      return res.json({
        data: memberships,
        page_info: {
          has_next_page: storedMembers.length > limit,
          has_previous_page: false
        }
      });
    } catch (error) {
